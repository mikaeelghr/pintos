...
struct dir *
get_dir_from_fd (struct list *l, int the_fd)
{
  struct file_descriptor *file_descriptor_instance = get_file_descriptor_from_fd (l, the_fd);
  if (file_descriptor_instance == NULL)
    return NULL;
  return file_descriptor_instance->dir;
}

struct file_descriptor*
create_file_descriptor_from_file (struct list *l, struct file* file)
{
  ...
  fds->file = file;
  fds->dir = NULL;
  list_push_back (l, &(fds->elem));
}

struct file_descriptor*
create_file_descriptor_from_dir (struct list *l, struct dir* dir)
{
  struct file_descriptor *fds = malloc(sizeof (struct file_descriptor));
  fds->fd = fdall;
  fdall += 1;
...
bool
remove_file_descriptor (struct list *l, int the_fd)
{
